...
<p align="center">
  <code>Konuşma Tanıma (Whisper)</code> → <code>Çeviri (Google Translate)</code> → <code>Seslendirme (Edge Neural TTS)</code> → <code>MP4 Çıktı</code>
</p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Edge_TTS-Neural-22c55e?style=for-the-badge&logo=microsoft&logoColor=white" />
</p>
...
3. **Seslendirme (TTS)** — [Edge TTS](https://github.com/rany2/edge-tts) ile Microsoft Neural ses motorunu kullanarak doğal Türkçe ses üretir. Erkek (Ahmet) ve kadın (Emel) ses seçenekleri vardır.
...
| **İnternet** | ✅ Gerekli | Çeviri API'leri ve Edge TTS bulut tabanlıdır |
...
└── backend/                   # 🐍 Python FastAPI sunucusu
    ...
    ├── 📄 speak.py            # Edge Neural TTS: metin → Türkçe ses (Ahmet / Emel)
...
| `speak.py` | **TTS (Text-to-Speech) modülü.** `edge-tts` ile Microsoft Neural ses motorunu kullanır. Türkçe erkek (Ahmet) ve kadın (Emel) sesleri desteklenir. Konuşma hızı ayarlanabilir (+0% ile +80% arası). |
...
| `edge-tts` | Microsoft Edge Neural TTS (bulut tabanlı, ücretsiz) |
...
curl -X POST http://127.0.0.1:8000/api/dub/start-url \
  ...
  -d '{"video_url": "https://www.youtube.com/watch?v=XXXXX", "rate": "+40%", "voice": "tr-TR-AhmetNeural"}'
...
<p align="center">
  <strong>talhaeens</strong> · <a href="https://talhaeens-ivy.netlify.app/">talhaeens-ivy.netlify.app</a>
</p>
